...
func TestTracing(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			assertFn: func(t *testing.T, trace string) {
				...
				assert.Regexp(t, `({"key":"service.name","value":{"stringValue":"WebhookX"}})`, trace)
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			chain := alice.New(observability.WrapEntryPointHandler(context.Background(), newTracing, "test"))
			epHandler := chain.Then(service)
