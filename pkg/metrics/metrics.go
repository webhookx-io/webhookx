...
type Metrics struct {
	...
	//ProxyRequestBytesCounter      metrics.Counter
	//ProxyResponseBytesCounter     metrics.Counter
...
func New(cfg config.MetricsConfig) (*Metrics, error) {
	...
	if len(cfg.Exporters) > 0 {
		...
		schedule.Schedule(m.ctx, m.collectRuntimeStats, time.Second*10)
	}
