...
func (q *RedisTaskQueue) Add(ctx context.Context, tasks []*TaskMessage) error {
	if tracer := tracing.TracerFromContext(ctx); tracer != nil {
		tracingCtx, span := tracer.Start(ctx, "taskqueue.redis.add", trace.WithSpanKind(trace.SpanKindServer))
		defer span.End()
		ctx = tracingCtx
	}
...
func (q *RedisTaskQueue) Get(ctx context.Context, opts *GetOptions) ([]*TaskMessage, error) {
	if tracer := tracing.TracerFromContext(ctx); tracer != nil {
		tracingCtx, span := tracer.Start(ctx, "taskqueue.redis.get", trace.WithSpanKind(trace.SpanKindServer))
		defer span.End()
		ctx = tracingCtx
	}
...
func (q *RedisTaskQueue) Delete(ctx context.Context, task *TaskMessage) error {
	if tracer := tracing.TracerFromContext(ctx); tracer != nil {
		tracingCtx, span := tracer.Start(ctx, "taskqueue.redis.delete", trace.WithSpanKind(trace.SpanKindServer))
		defer span.End()
		ctx = tracingCtx
	}
